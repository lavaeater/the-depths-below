...
object MarchingCubes {
    ...
    fun marchingCubesChar(
        ...
        val maxX = voxDim[0] * (volDim[0] - 1)
        val maxY = voxDim[1] * (volDim[1] - 1)
        val maxZ = voxDim[2] * (volZFull - 1)
        val maxAxisVal = Math.max(maxX, Math.max(maxY, maxZ))

        // Volume iteration
        for (z in 0 until volDim[2] - 1) {
            for (y in 0 until volDim[1] - 1) {
                for (x in 0 until volDim[0] - 1) {
                    ...
                    //              |/__________________|/
                    //             p                     px
                    val p = x + volDim[0] * y + volDim[0] * volDim[1] * (z + offset)
                    val px = p + 1
                    val py = p + volDim[0]
                    val pxy = py + 1
                    val pz = p + volDim[0] * volDim[1]
                    val pxz = px + volDim[0] * volDim[1]
                    val pyz = py + volDim[0] * volDim[1]
                    val pxyz = pxy + volDim[0] * volDim[1]

                    //							  X              Y                    Z
                    val position = floatArrayOf(x * voxDim[0], y * voxDim[1], (z + offset) * voxDim[2])
                    ...
                    // bottom of the cube
                    if (bits and 1 != 0) {
                        mu = ((isoLevel.code - value0.code) / (value1.code - value0.code)).toFloat()
                        vertList[0] = lerp(
                    ...
                    if (bits and 2 != 0) {
                        mu = ((isoLevel.code - value1.code) / (value3.code - value1.code)).toFloat()
                        vertList[1] = lerp(
                            floatArrayOf(
                                position[0] + voxDim[0],
                                position[1], position[2]
                    ...
                    if (bits and 4 != 0) {
                        mu = ((isoLevel.code - value2.code) / (value3.code - value2.code)).toFloat()
                        vertList[2] = lerp(
                            floatArrayOf(
                    ...
                    if (bits and 8 != 0) {
                        mu = ((isoLevel.code - value0.code) / (value2.code - value0.code)).toFloat()
                        vertList[3] = lerp(
                            position, floatArrayOf(
                                position[0], position[1] + voxDim[1],
                                position[2]
                    ...
                    // top of the cube
                    if (bits and 16 != 0) {
                        mu = ((isoLevel.code - value4.code) / (value5.code - value4.code)).toFloat()
                        vertList[4] = lerp(
                    ...
                    if (bits and 32 != 0) {
                        mu = ((isoLevel.code - value5.code) / (value7.code - value5.code)).toFloat()
                        vertList[5] = lerp(
                            floatArrayOf(
                                position[0] + voxDim[0],
                    ...
                    if (bits and 64 != 0) {
                        mu = ((isoLevel.code - value6.code) / (value7.code - value6.code)).toFloat()
                        vertList[6] = lerp(
                    ...
                    if (bits and 128 != 0) {
                        mu = ((isoLevel.code - value4.code) / (value6.code - value4.code)).toFloat()
                        vertList[7] = lerp(
                            floatArrayOf(position[0], position[1], position[2] + voxDim[2]), floatArrayOf(
                    ...
                    if (bits and 256 != 0) {
                        mu = ((isoLevel.code - value0.code) / (value4.code - value0.code)).toFloat()
                        vertList[8] = lerp(
                            position, floatArrayOf(
                                position[0],
                                position[1], position[2] + voxDim[2]
                    ...
                    if (bits and 512 != 0) {
                        mu = ((isoLevel.code - value1.code) / (value5.code - value1.code)).toFloat()
                        vertList[9] = lerp(
                            floatArrayOf(
                                position[0] + voxDim[0],
                                position[1], position[2]
                    ...
                    if (bits and 1024 != 0) {
                        mu = ((isoLevel.code - value3.code) / (value7.code - value3.code)).toFloat()
                        vertList[10] = lerp(
                            floatArrayOf(
                    ...
                    if (bits and 2048 != 0) {
                        mu = ((isoLevel.code - value2.code) / (value6.code - value2.code)).toFloat()
                        vertList[11] = lerp(
                            floatArrayOf(
                                position[0], position[1] + voxDim[1],
                                position[2]
                    ...
                    while (TablesMC.MC_TRI_TABLE[cubeindex + i] != -1) {
                        ...
                        vertices.add(
                            floatArrayOf(
                                vertList[index3][0] / maxAxisVal - 0.5f,
                                vertList[index3][1] / maxAxisVal - 0.5f,
                                vertList[index3][2] / maxAxisVal - 0.5f
                            )
                        )
                        vertices.add(
                            floatArrayOf(
                                vertList[index2][0] / maxAxisVal - 0.5f,
                                vertList[index2][1] / maxAxisVal - 0.5f,
                                vertList[index2][2] / maxAxisVal - 0.5f
                            )
                        )
                        vertices.add(
                            floatArrayOf(
                                vertList[index1][0] / maxAxisVal - 0.5f,
                                vertList[index1][1] / maxAxisVal - 0.5f,
                                vertList[index1][2] / maxAxisVal - 0.5f
                            )
                        )
                        i += 3
                    }
        ...
        callback.vertices = vertices
        callback.run()
    }

    fun marchingCubesShort(
        values: ShortArray,
        volDim: IntArray,
        volZFull: Int,
        voxDim: FloatArray,
        ...
        for (z in 0 until size - 1) {
            for (y in 0 until size - 1) {
                for (x in 0 until size - 1) {
                    ...
                    //              |/__________________|/
                    //             p                     px
                    val p = x + size * y + size * size * (z + offset)
                    val px = p + 1
                    val py = p + size
                    val pxy = py + 1
                    val pz = p + size * size
